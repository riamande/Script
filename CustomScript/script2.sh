...
i=0; while [ $i -lt 500 ]; do mysql -u kaskus_fight -ptryITharder1990 -h 172.20.0.153 kaskus_pm -r -s -N -e "select concat(userid,':',count(pm_id)) from pm_user_$i where date>='$lastweek_date' and date<='$tanggal_date' group by userid;" >> $temp_mongo_script/temp.log; i=`expr $i + 1`; done;

mysql -u$MYSQL_USER -p$MYSQL_PASS -h 172.20.0.73 -e "USE user; CREATE TABLE temp(userid int primary key,totalpost int);"

mysqlimport -u$MYSQL_USER -p$MYSQL_PASS -h 172.20.0.73 --local --fields-terminated-by=':' user $temp_mongo_script/temp.log

mysql -u$MYSQL_USER -p$MYSQL_PASS -h 172.20.0.73 -s -N -e "use user; select concat(if(b.usergroupid=2,c.title,if(b.usergroupid=3,c.title,if(b.usergroupid=16,c.title,if(b.usergroupid=23,c.title,if(b.usergroupid=36,c.title,if(b.usergroupid in (5,7,19,24,29,30,37),'Moderator','Special Users')))))),':',count(a.userid),':',sum(a.totalpost)) from temp a join userinfo b on a.userid=b.userid join usergroup c on c.usergroupid = b.usergroupid group by (if(b.usergroupid=2,c.title,if(b.usergroupid=3,c.title,if(b.usergroupid=16,c.title,if(b.usergroupid=23,c.title,if(b.usergroupid=36,c.title,if(b.usergroupid in (5,7,19,24,29,30,37),'Moderator','Special Users')))))));" > $temp_mongo_script/pm_statistic.log

mysql -u$MYSQL_USER -p$MYSQL_PASS -h 172.20.0.73 -s -N -e "use user;drop table temp;"
