...
/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.find({forum_id:{\$in:[$id_fjb]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek} },{_id:0,post_userid:1}).forEach(printjson)"	 |sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' >> $url/"userid2_"$condition".log"
...
then
	...
	/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.find({forum_id:{\$in:[$id_fjb]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek},prefix_id:{\$in:['wts','wtb','WTS','WTB']}},{_id:1}).forEach(printjson)"|grep -v Mongo|grep -v connect|cut -d':' -f2|cut -d'(' -f2|cut -d")" -f1 |sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' >> $url/"thread_id_"$condition".log"
...
#		/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.post.find({thread_id:{\$in:[$id]}},{_id:0,post_userid:1}).forEach(printjson)" |sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' >> $url/"thread_id_temp_"$condition".log"	
...
for i in `mysql -u$username -p$password -h $host_user -s -N -e "use kaskus_statistic;select forum_id,id_report from report_info where id_report like 'KK-$code-FJB-10-%';"|tr '\t' '.'`; do
	...
	/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.find({forum_id:{\$in:[$forum_child]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek},prefix_id:{\$in:['WTB','wtb']}},{_id:0,post_userid:1,forum_id:1}).forEach(printjson)" |sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' > $url/"wtb_"$condition".log";
...
do
	...
	a=`echo $i|cut -d'.' -f1`
forum_child=`mysql -u$username -p$password -h $host_forum -s -N -e "use forum; select child_list from forum_list where forum_id = $a"`
	/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.find({forum_id:{\$in:[$forum_child]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek},prefix_id:{\$in:['WTS','wts']}},{_id:0,post_userid:1,forum_id:1}).forEach(printjson)" |sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' >$url/"wts_"$condition".log";

	total=`cat  $url/"wts_"$condition".log"|grep -v Mongo |grep -v connect|cut -d',' -f2|cut -d':' -f2 |cut -d'"' -f2|wc -l`;
	echo $kode"="$total >> $url/"Fjbreport_"$tanggal"_"$condition".log";
done
/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.count({forum_id:{\$in:[$id_fjb]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek},prefix_id:{\$nin:['wtb','WTB','wts','WTS']}})"|sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' > $url/"tempunidentified_"$condition".log"
...
do
	...
	total_post=0;
	forum_child=`mysql -u$username -p$password -h $host_forum -s -N -e "use forum; select child_list from forum_list where forum_id = $a"`
	/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.find({forum_id:{\$in:[$forum_child]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek},prefix_id:{\$in:['WTB','wtb']},reply_count:{\$gt:0}},{_id:0,reply_count:1,forum_id:1}).forEach(printjson)" |sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' > $url/"wtb_"$condition".log";
...
do
	...
	/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.find({forum_id:{\$in:[$forum_child]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek},prefix_id:{\$in:['WTS','wts']},reply_count:{\$gt:0}},{_id:0,reply_count:1,forum_id:1}).forEach(printjson)" |sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' > $url/"wtb_"$condition".log";
	for i in `cat $url/"wtb_"$condition".log" |grep -v Mongo|grep -v connect|cut -d',' -f2|cut -d':' -f2|cut -d' ' -f2`
	do
		 total_post=`expr $total_post + $i`;
	done
...
/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.find({forum_id:{\$in:[$forum_child]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek},prefix_id:{\$nin:['WTS','wts','WTB','wtb']},reply_count:{\$gt:0}},{_id:0,reply_count:1,forum_id:1}).forEach(printjson)" |sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' > $url/"wtb_"$condition".log";
...
id_fjb=`mysql -u$username -p$password -h $host_forum forum -s -N -e "select child_list from forum_list where forum_id=25;" |sed -e 's$\,\-1$$g'`
/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.find({forum_id:{\$nin:[$id_fjb]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek} },{_id:0,post_userid:1}).forEach(printjson)" |sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' >> $url/"userid2_"$condition".log"
cat $url/"userid2_"$condition".log" |sort|uniq >> $url/"userid_"$condition".log"
cat $url/"userid_"$condition".log"|grep post_userid |cut -d':' -f2|cut -d' ' -f2|cut -d'"' -f2|sort|uniq|head -c-1 >> $url/"distinctuserid_"$condition".log"
...
#replies umur
/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.find({forum_id:{\$nin:[$id_fjb]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek}},{_id:1}).forEach(printjson)"|grep -v Mongo|grep -v connect|cut -d':' -f2|cut -d'(' -f2|cut -d")" -f1 >> $url/"thread_id_"$condition".log"
...
do
	...
	a=`echo $i|cut -d'.' -f1`;
	forum_child=`mysql -u$username -p$password -h $host_forum -s -N -e "use forum; select child_list from forum_list where forum_id = $a"`
	/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.count({forum_id:{\$in:[$forum_child]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek}})"|grep -v MongoDB |grep -v connect |sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' > $url/"temp_"$condition".log";
...
do
	...
	/opt/mongodb_3.0.10/bin/mongo 127.0.0.1:27018/kaskus_forum -uforumshardrw -pG5NVEI5WkLFgGTB1 --eval "db.thread.find({forum_id:{\$in:[$forum_child]},sticky:0,dateline:{\$lte:$tanggal,\$gte:$lastweek},reply_count:{\$gt:0}},{_id:0,reply_count:1,forum_id:1}).forEach(printjson)" |sed 's/\<NumberLong\>//g'| sed 's/[\)(]//g' > $url/"wtb_"$condition".log";
	for i in `cat $url/"wtb_"$condition".log" |grep -v Mongo|grep -v connect|cut -d',' -f2|cut -d':' -f2|cut -d' ' -f2`
	do
		 total_post=`expr $total_post + $i`;
	done
