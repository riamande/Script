ipdb=$1
totaldb=`mysql -h $ipdb -ukaskus_fight -ptryITharder1990 -s -N -e "show databases;" |grep -v information_schema |wc -l `
echo $totaldb
for ((i=1;i<=$totaldb;i++))
do
...
countrow=`mysql -h $ipdb -ukaskus_fight -ptryITharder1990 $dbname -s -N -e "select count(*) from $tablename;"`
echo $tablename $countrow
echo "$dbname.$tablename : $countrow" >> list_table_count.txt
done
done
