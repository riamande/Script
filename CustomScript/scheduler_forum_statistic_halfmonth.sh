...
### daily userlogin ###
mongo 172.20.0.91/kaskus_user_log --eval "rs.slaveOk();db.userloginlog.find({_id:{\$gte:ObjectId('$OID_START'),\$lt:ObjectId('$OID_END')},referer:{\$nin:[/fjb./]}},{_id:0,userid:1}).forEach(printjson)" |grep -v 'MongoDB shell version:\|connecting to:' |cut -d '"' -f4 |sort |uniq > $datadir/temp_forum1.temp
...
# friends
mongo 172.20.0.242/kaskus_friend --eval "rs.slaveOk();db.friend_friends.find({since:{\$gte:$day_start,\$lt:$day_end}},{_id:0,user_id:1}).forEach(printjson)" |grep -v 'MongoDB shell version:\|connecting to:' > $datadir/temp_forum33.temp
...
### monthly login ###
mongo 172.20.0.91/kaskus_user_log --eval "rs.slaveOk();db.userloginlog.find({_id:{\$gte:ObjectId('$OID_START_MONTH'),\$lt:ObjectId('$OID_END_MONTH')},referer:{\$nin:[/fjb./]}},{_id:0,userid:1}).forEach(printjson)" |grep -v 'MongoDB shell version:\|connecting to:' |cut -d '"' -f4 |sort |uniq |wc -l > $datadir/temp_forummonth1.temp
...
### monthly subscribers ###
# forum
mongo 172.20.0.242/kaskus_forum1 --eval "rs.slaveOk();db.subscribeforum.find({_id:{\$gte:ObjectId('$OID_START_MONTH'),\$lt:ObjectId('$OID_END_MONTH')},is_fjb:0},{_id:0,userid:1}).forEach(printjson)" |grep -v 'MongoDB shell version:\|connecting to:' |sort |uniq |wc -l > $datadir/temp_forummonth21.temp
total_month31=`cat $datadir/temp_forummonth21.temp`
# thread
mongo 172.20.0.242/kaskus_forum1 --eval "rs.slaveOk();db.subscribethread.find({_id:{\$gte:ObjectId('$OID_START_MONTH'),\$lt:ObjectId('$OID_END_MONTH')},is_fjb:0},{_id:0,userid:1}).forEach(printjson)" |grep -v 'MongoDB shell version:\|connecting to:' |sort |uniq |wc -l > $datadir/temp_forummonth22.temp
total_month32=`cat $datadir/temp_forummonth22.temp`
# friends
mongo 172.20.0.242/kaskus_friend --eval "rs.slaveOk();db.friend_friends.find({since:{\$gte:$month_start,\$lt:$month_end}},{_id:0,user_id:1}).forEach(printjson)" |grep -v 'MongoDB shell version:\|connecting to:' |sort |uniq |wc -l > $datadir/temp_forummonth23.temp
...
mysql -h 172.20.0.159 -ubackup -pkaskus test -e "truncate temp_monthlyforum;"
